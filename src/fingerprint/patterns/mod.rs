...
//! - Simple substring / escaped-literal matching
...
/// Minimum Wappalyzer `\\;confidence:N` required for a pattern to match.
pub(crate) const MIN_PATTERN_CONFIDENCE: u32 = 50;
...
struct ParsedPattern {
    ...
    /// Upstream confidence 0–100; defaults to 100 when omitted.
    confidence: u32,
}
...
fn parse_pattern(pattern: &str) -> ParsedPattern {
    ...
    // Find version / confidence metadata in subsequent parts.
    ...
    for part in parts.iter().skip(1) {
        if let Some(colon_pos) = part.find(':') {
            ...
            if key == "version" && version_template.is_none() {
                // First "version:" wins (wappalyzergo order).
...
pub(crate) fn matches_pattern(pattern: &str, text: &str) -> PatternMatchResult {
    ...
    if is_regex {
        ...
        let Some(re) = get_or_compile_regex(&parsed.pattern_for_match, &cache_key) else {
            // Invalid regex patterns are skipped (no silent substring match).
            return PatternMatchResult {
                matched: false,
                version: None,
            };
        ...
        let Some(re) = get_or_compile_regex(&escaped_pattern, &cache_key) else {
            // Escaped literal should always compile; if not, skip the pattern.
            return PatternMatchResult {
                matched: false,
                version: None,
            };
...
// Version template helpers live in `version`.
use version::*;
...
mod tests {
    ...
    #[test]
    fn test_matches_pattern_invalid_regex_is_skipped() {
        // Invalid regex patterns must not silently substring-match.
        ...
        assert!(!matches_pattern(r"\invalid", r"text with \invalid").matched);
        assert!(!matches_pattern("(unclosed", "text with (unclosed paren").matched);

        // `{` is not treated as regex syntax here, so this is an escaped literal match.
    ...
    #[test]
    fn test_parse_pattern_respects_confidence_threshold() {
        // Below threshold → no match
        let low = matches_pattern(r"jquery\;confidence:49", "jquery.min.js");
        assert!(!low.matched);

        // At/above threshold → match
        let ok = matches_pattern(r"jquery\;confidence:50", "jquery.min.js");
        assert!(ok.matched);
        assert_eq!(ok.version, None);
    }
    ...
    #[test]
    fn test_get_or_compile_regex_skips_invalid_regex() {
        // Invalid regex must not silently substring-match
