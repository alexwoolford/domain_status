...
pub(crate) const LOGGING_INTERVAL: usize = 5;
...
pub(crate) const URL_PROCESSING_TIMEOUT: Duration = Duration::from_secs(35);
...
pub(crate) const DNS_TIMEOUT_SECS: u64 = 3;
...
pub(crate) const MAX_TXT_RECORD_SIZE: usize = 1024;
...
pub(crate) const MAX_TXT_RECORD_COUNT: usize = 20;

/// TCP connection timeout in seconds
pub(crate) const TCP_CONNECT_TIMEOUT_SECS: u64 = 5;
/// TLS handshake timeout in seconds
pub(crate) const TLS_HANDSHAKE_TIMEOUT_SECS: u64 = 5;
...
pub(crate) const MAX_FAVICON_SIZE: usize = 50 * 1024;
/// Favicon fetch timeout in seconds
/// Favicons are small files; 5s is generous for a single image download
pub(crate) const FAVICON_FETCH_TIMEOUT_SECS: u64 = 5;

// HTML text extraction limits
/// Maximum HTML preview length in characters for debugging (500 chars).
/// Referenced by unit tests that lock preview truncation behavior.
#[allow(dead_code)] // exercised from `#[cfg(test)]` helpers; kept as shared limit
pub(crate) const MAX_HTML_PREVIEW_CHARS: usize = 500;
...
pub(crate) const MAX_ERROR_MESSAGE_LENGTH: usize = 2000;
...
pub(crate) const MAX_HEADER_VALUE_LENGTH: usize = 1000;
...
pub(crate) const RETRY_INITIAL_DELAY_MS: u64 = 500;
...
pub(crate) const RETRY_MAX_DELAY_SECS: u64 = 15;
...
pub(crate) const STATUS_SERVER_LOGGING_INTERVAL_SECS: u64 = 30;

// HTTP status codes (for clarity and consistency)
/// HTTP status code for "Too Many Requests" (rate limiting)
pub(crate) const HTTP_STATUS_TOO_MANY_REQUESTS: u16 = 429;
...
pub(crate) const MAX_RULESET_DOWNLOAD_SIZE: usize = 10 * 1024 * 1024;
/// Maximum size for `GeoIP` database downloads in bytes (100MB)
/// `GeoIP` databases are large but should not exceed this limit
pub(crate) const MAX_GEOIP_DOWNLOAD_SIZE: usize = 100 * 1024 * 1024;
...
pub(crate) const MAX_GEOIP_ARCHIVE_ENTRY_SIZE: usize = 100 * 1024 * 1024;
...
pub(crate) const MAX_WHOIS_CACHE_ENTRIES: usize = 50_000;
...
pub(crate) const MAX_WHOIS_RAW_TEXT_SIZE: usize = 256 * 1024;
...
pub(crate) const MAX_WHOIS_CACHE_FILE_SIZE: u64 = 512 * 1024;
/// User-Agent cache TTL: 30 days
/// Chrome releases roughly every 4 weeks, so 30 days ensures we stay current
pub(crate) const USER_AGENT_CACHE_TTL_SECS: u64 = 30 * 24 * 60 * 60;
/// `GeoIP` cache TTL: 7 days
pub(crate) const GEOIP_CACHE_TTL_SECS: u64 = 7 * 24 * 60 * 60;
...
pub(crate) const MAX_EXPORT_RELATED_RECORDS: i64 = 500;
