-- Remove deprecated fingerprints_source and fingerprints_version columns from url_status
-- These fields are now stored at the run level in the runs table and should be queried from there.
-- Keeping them in url_status was causing confusion and data duplication.

-- Note: SQLite doesn't support DROP COLUMN directly, so we recreate the table

-- Step 1: Create new table without deprecated fingerprint columns
CREATE TABLE IF NOT EXISTS url_status_new (
    ...
    key_algorithm TEXT,
    run_id TEXT,
    -- Removed: fingerprints_source (use runs.fingerprints_source)
    -- Removed: fingerprints_version (use runs.fingerprints_version)
    UNIQUE (final_domain, timestamp)
);

-- Step 2: Copy data (excluding deprecated fingerprint columns)
