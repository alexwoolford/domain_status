...
SQLite with WAL mode. Schema in `migrations/` (15 migration files, `0001`–`0015`). Main tables: `runs`, `url_status` (fact table), `url_failures`, plus satellite tables for DNS, TLS, headers, technologies, WHOIS, GeoIP, secrets, etc. (see `URL_STATUS_CORE_SATELLITE_TABLES` / `URL_STATUS_ENRICHMENT_SATELLITE_TABLES` and `DATABASE.md`).
